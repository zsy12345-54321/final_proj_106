...
class PIDJointVelocityController(Controller):
    ...
    def step_control(self, target_position, target_velocity, target_acceleration):
        ...
        self.integ_error = position_error + self.Kw * self.integ_error

        # Calculate PID control output
        controller_velocity = (
        target_velocity+
            self.Kp @ position_error +        # Proportional term
            self.Ki @ self.integ_error +      # Integral term
            self.Kd @ velocity_error          # Derivative term
        )

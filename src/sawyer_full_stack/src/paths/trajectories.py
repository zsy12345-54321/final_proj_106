    ...
    def target_pose(self, time):
        ...
        if time <= self.total_time / 2.0:
            t = time
                ...
                #pos = (self.start_position + 0.5 * self.acceleration * t**2 * (self_goal_position -self.start_position)      (0.5*self.total_time**2))
            pos = self.start_position + 0.5 * self.acceleration * t**2
        else:
            # TODO: Calculate the position of the end effector at time t, 
            ...
            #remaining_distance = self.distance / 2.0  # Remaining distance to cover in second half
            #pos = self.start_position + 0.5 * self.acceleration * t**2 + remaining_distance + self.v_max * t - 0.5 * self.acceleration * t**2
            pos = (self.start_position + self.distance / 2.0 + 
                   self.v_max * t - 0.5 * self.acceleration * t**2)
...
class CircularTrajectory(Trajectory):
    ...
    def target_pose(self, time):
        ...
        pos_d = self.center_position + self.radius * np.array([np.cos(theta), np.sin(theta), 0])
    ...
    def target_velocity(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            t = time - self.total_time / 2.0
            theta = (np.pi) + self.angular_v_max * t - 0.5 * self.angular_acceleration * t**2
            theta_dot = max(0, self.angular_v_max - self.angular_acceleration * t)

        # Compute linear velocity based on angular velocity
        vel_d = self.radius * theta_dot * np.array([-np.sin(theta), np.cos(theta), 0])
